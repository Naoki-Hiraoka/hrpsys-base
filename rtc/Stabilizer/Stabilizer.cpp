...
RTC::ReturnCode_t Stabilizer::onInitialize()
{
  ...
  eefm_use_quaternion_body_attitude_control = false;
...
{
    if(st_algorithm == OpenHRP::StabilizerService::MCS){
        is_emergency = multicontactstabilizer.calcStateForEmergencySignal(emergency_check_mode,on_ground, transition_count, control_mode==MODE_ST);
        return;
    }
  // COP Check
...
void Stabilizer::sync_2_st ()
{
  ...
  rdx = rdy = rx = ry = 0;
  d_rpy[0] = d_rpy[1] = 0;
  d_quaternion[0] = d_quaternion[1] = d_quaternion[2] = d_quaternion[3] = 0;
  pdr = hrp::Vector3::Zero();
